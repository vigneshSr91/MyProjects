...
def index():
    if request.method == 'POST':
        ...
        if 1 == 1:
            ...
            reviews = db.get_collection(searchString)
            if reviews.count_documents({}) > 0: # if there is a collection with searched keyword and it has records in it
                ...
                bigboxes = flipkart_html.findAll("div", {"class": "_1AtVbE col-12-12"}) #{"class": "bhgxx2 col-12-12"}) # seacrhing for appropriate tag to redirect to the product link
                del bigboxes[0:3] # the first 3 members of the list do not contain relevant information, hence deleting them.
                box = bigboxes[0] #  taking the first iteration (for demo)
                productLink = "https://www.flipkart.com" + box.div.div.div.a['href'] # extracting the actual product link
                prodRes = requests.get(productLink) # getting the product page from server
                prod_html = bs(prodRes.text, "html.parser") # parsing the product page as HTML
                commentboxes = prod_html.find_all('div', {'class': "_2c2kV-"}) #{'class': "_3nrCtb"}) finding the HTML section containing the customer comments
